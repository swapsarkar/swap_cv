...
class Generator(nn.Module):
    def __init__(self, z_dim, in_channels, img_channels=3):
        ...
        for i in range(len(factors) - 1):
            ...
            self.prog_blocks.append(ConvBlock(conv_in_channel, conv_out_channel))
            self.rgb_layers.append(
                WSConv2d(conv_out_channel, img_channels, kernel_size=1, stride=1, padding=0)
            )
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, alpha, steps):
        current_steps = len(self.prog_blocks) - steps
        out = self.leaky(self.rgb_layers[current_steps](x))

        if steps == 0:
            out = self.minibatch_std(out)
            return self.last_block(out).view(out.shape[0], -1)

        downscaled = self.leaky(self.rgb_layers[current_steps + 1](self.avg_pool(x)))
        out = self.avg_pool(self.prog_blocks[current_steps](out))
        out = self.fade_in(alpha, downscaled, out)

        for steps in range(current_steps + 1, len(self.prog_blocks)):
            out = self.prog_blocks[steps](out)
